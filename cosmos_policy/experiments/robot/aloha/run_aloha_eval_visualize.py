...
"""
run_aloha_eval_visualize.py

Visualizes model predictions on ALOHA training dataset episodes without running simulation rollouts.
...
Usage example:
    uv run -m cosmos_policy.experiments.robot.aloha.run_aloha_eval_visualize \
        --config cosmos_predict2_2b_480p_aloha_185_demos_4_tasks_mixture_foldshirt15_candiesinbowl45_candyinbag45_eggplantchickenonplate80 \
...
class PolicyEvalVisualizeConfig:
    ...
    num_third_person_images: int = 1                                     # Number of third-person images to include in input (ALOHA: 1 cam_high image)
    use_wrist_image: bool = True                                         # Whether to include wrist image in input
    num_wrist_images: int = 2                                            # Number of wrist images to include in input (ALOHA: 2 wrist images - left and right)
    use_proprio: bool = True                                             # Whether to include proprio state in input
    flip_images: bool = False                                            # Whether to flip images vertically across x-axis (ALOHA typically doesn't need flipping)
    ...
    chunk_size: int = 50                                                 # Number of actions to predict in chunk (ALOHA default: 50)
    ...
    n: Optional[int] = None                                              # Maximum number of frames to visualize per episode (None = all)
...
def prepare_observation_from_data(primary_image, left_wrist_image, right_wrist_image, proprio, flip_images: bool = False):
    """Prepare observation dictionary from training data."""
    # Flip images if needed
    if flip_images:
        primary_image = np.flipud(primary_image)
        left_wrist_image = np.flipud(left_wrist_image)
        right_wrist_image = np.flipud(right_wrist_image)
...
def create_side_by_side_frame(gt_primary: np.ndarray, gt_left_wrist: np.ndarray, gt_right_wrist: np.ndarray,
                             pred_primary: np.ndarray, pred_left_wrist: np.ndarray, pred_right_wrist: np.ndarray,
    ...
    Layout:
      ...
      Middle row: [GT Left Wrist] | [Predicted Left Wrist]
      Bottom row: [GT Right Wrist + action plot] | [Predicted Right Wrist + action plot]
    """
    ...
    gt_left_wrist_resized = resize_img(gt_left_wrist, target_size)
    gt_right_wrist_resized = resize_img(gt_right_wrist, target_size)
    pred_primary_resized = resize_img(pred_primary, target_size)
    pred_left_wrist_resized = resize_img(pred_left_wrist, target_size)
    pred_right_wrist_resized = resize_img(pred_right_wrist, target_size)

    # Add text overlays
    gt_primary_with_text = add_text_overlays(gt_primary_resized, timestep, gt_value, None)
    gt_left_wrist_with_text = add_text_overlays(gt_left_wrist_resized, timestep, gt_value, None)
    gt_right_wrist_with_text = add_text_overlays(gt_right_wrist_resized, timestep, gt_value, None)
    pred_primary_with_text = add_text_overlays(pred_primary_resized, timestep, None, pred_value)
    pred_left_wrist_with_text = add_text_overlays(pred_left_wrist_resized, timestep, None, pred_value)
    pred_right_wrist_with_text = add_text_overlays(pred_right_wrist_resized, timestep, None, pred_value)
    
    # Create side-by-side layout
    # Top row: GT Primary | Predicted Primary
    # Middle row: GT Left Wrist | Predicted Left Wrist
    # Bottom row: GT Right Wrist | Predicted Right Wrist
    top_row = np.hstack([gt_primary_with_text, pred_primary_with_text])
    middle_row = np.hstack([gt_left_wrist_with_text, pred_left_wrist_with_text])
    bottom_row = np.hstack([gt_right_wrist_with_text, pred_right_wrist_with_text])
    
    # Add action trajectories (history plot) on bottom row if provided
    ...
    frame = np.vstack([top_row, middle_row, bottom_row])
...
def visualize_episode_predictions(
    ...
    gt_left_wrist_images = episode_data["left_wrist_images"]  # (T, H, W, 3)
    gt_right_wrist_images = episode_data["right_wrist_images"]  # (T, H, W, 3)
    ...
    pred_actions_all = []   # one action per timestep (for history plot)
    pred_values_all = []
    ...
    for t in tqdm.tqdm(range(num_steps)):
        ...
        observation = prepare_observation_from_data(
            gt_images[t],
            gt_left_wrist_images[t],
            gt_right_wrist_images[t],
            gt_proprio[t],
            cfg.flip_images
        )
        ...
        # Store predictions
        # get_action returns actions as a list of lists: [[action_0], ..., [action_chunk_size-1]]
        if isinstance(pred_actions_chunk, list) and len(pred_actions_chunk) > 0:
            chunk_arr = np.array(pred_actions_chunk)
            if chunk_arr.ndim == 3:   # (chunk_size, 1, action_dim) → (chunk_size, action_dim)
                chunk_arr = chunk_arr.squeeze(1)
            elif chunk_arr.ndim == 1:
                chunk_arr = chunk_arr.reshape(1, -1)
            # First action for history plot
            pred_actions_all.append(chunk_arr[0])
        else:
            zeros = np.zeros_like(gt_actions[0])
            pred_actions_all.append(zeros)
        ...
        pred_left_wrist = pred_future_images.get("future_wrist_image", None) if pred_future_images else None
        pred_right_wrist = pred_future_images.get("future_wrist_image2", None) if pred_future_images else None
        ...
        if pred_left_wrist is not None:
            if isinstance(pred_left_wrist, torch.Tensor):
                pred_left_wrist = pred_left_wrist.cpu().numpy()
            pred_left_wrist = np.array(pred_left_wrist)
            # Remove batch dimension if present (B, H, W, C) -> (H, W, C)
            if pred_left_wrist.ndim == 4:
                pred_left_wrist = pred_left_wrist[0]
        
        if pred_right_wrist is not None:
            if isinstance(pred_right_wrist, torch.Tensor):
                pred_right_wrist = pred_right_wrist.cpu().numpy()
            pred_right_wrist = np.array(pred_right_wrist)
            # Remove batch dimension if present (B, H, W, C) -> (H, W, C)
            if pred_right_wrist.ndim == 4:
                pred_right_wrist = pred_right_wrist[0]
        ...
        if pred_left_wrist is None:
            pred_left_wrist = gt_left_wrist_images[min(t + 1, num_steps - 1)]
        if pred_right_wrist is None:
            pred_right_wrist = gt_right_wrist_images[min(t + 1, num_steps - 1)]
        ...
        if cfg.flip_images:
            ...
            if pred_left_wrist is not None:
                pred_left_wrist = np.flipud(pred_left_wrist)
            if pred_right_wrist is not None:
                pred_right_wrist = np.flipud(pred_right_wrist)
        
        # Build history arrays for trajectory plot (bottom row)
        ...
        frame = create_side_by_side_frame(
            ...
            gt_left_wrist=gt_left_wrist_images[t],
            gt_right_wrist=gt_right_wrist_images[t],
            pred_primary=pred_primary,
            pred_left_wrist=pred_left_wrist,
            pred_right_wrist=pred_right_wrist,
            timestep=t,
            gt_value=gt_value,
...
@draccus.wrap()
def eval_aloha_visualize(cfg: PolicyEvalVisualizeConfig):
